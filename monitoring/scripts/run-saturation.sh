...
if [ -z "${HOT_CHALLENGE_ID:-}" ]; then
  ...
  [ -n "$HOT_CHALLENGE_ID" ] || { echo "[FAIL] SAT_HOT_1 없음 — seed-saturation.sql 먼저 적용하세요." >&2; exit 1; }
fi
if [ -z "${BREADTH_MIN:-}" ] || [ -z "${BREADTH_MAX:-}" ]; then
  ...
  [ -n "$BREADTH_MIN" ] && [ -n "$BREADTH_MAX" ] || { echo "[FAIL] SAT_BREADTH_ 없음 — seed-saturation.sql 먼저 적용하세요." >&2; exit 1; }
fi
...
SCENARIO="$SCEN" OUT_JSON="$K6JSON" \
  ...
  k6 run "$ROOT/monitoring/k6/saturation-test.js" >"$K6OUT" 2>&1
