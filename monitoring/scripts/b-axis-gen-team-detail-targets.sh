#!/usr/bin/env bash
# b-axis-gen-team-detail-targets.sh — RT_ 시딩 결과에서 monitoring/k6/b-axis-team-detail-targets.json 을 생성한다.
#
# b-axis-seed-team-detail.sql 적용 직후 실행할 것. challenge id는 시퀀스 값이라
# 재시딩마다 바뀌므로, b-axis-team-detail-targets.json 을 수동 관리하지 말고 항상 이 스크립트로
# 재생성해야 한다(하드코딩된 옛 id가 남으면 b-axis-team-detail.js의 setup 로그인이
# 전부 실패한다).
#
# 앵커 계정 규약(시더와 동일해야 함):
...
#   password = seed1234
#
# Usage: monitoring/scripts/b-axis-gen-team-detail-targets.sh
...
OUT="${OUT:-$ROOT/monitoring/k6/b-axis-team-detail-targets.json}"
PASSWORD="seed1234"   # b-axis-seed-team-detail.sql 의 앵커 비밀번호와 동일해야 함
...
if [ -z "$ROWS" ]; then
  echo "[FAIL] RT_ 챌린지가 없습니다 — b-axis-seed-team-detail.sql 먼저 적용하세요." >&2
  exit 1
fi
