...
복귀 미션 API를 삭제했는데 앱이 아직 호출한다. **홈·인증 화면이 현재 404를 받는다.**

| # | 할 일 | 상세 |
|---|---|---|
| **P0-1** | `recovery_service.dart` · `models/recovery.dart` **삭제** + 호출부 제거 | §12 |
| **P0-2** | **상점을 서버 API로** — 구제권 구매는 이미 서버에 있다. 가격 100 ❌ → **800** ⭕. "사용" 버튼 제거 | **§S-6** |
| **P0-3** | **구제 안내 팝업** — 없으면 사용자가 새벽에 예고 없이 스트릭 0 + 코인 −500 | **§S-8.4** |

추가로 **카테고리를 한글로 보내고 있어 AI 인증이 현재 전부 실패**한다 → §4.1
화면 단위 상세·37개 API 매핑은 **§10 화면 명세** 에 있다.
...
11. 응답 래퍼가 축마다 다름 — A축 raw / B축 `{success,data}` (`MVP_API_SPEC.md` §S-1.2)
