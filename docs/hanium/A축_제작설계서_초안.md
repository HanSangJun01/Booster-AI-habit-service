...
| S/W | 복귀 미션 수행 | 마감 이내에 GPS 재인증으로 미인증일을 보정한다. 복귀 성공은 '복귀한 날의 인증'으로 완전 간주(F2 정책) → 코인 차감(-50, 잔액까지만), 미인증일과 오늘 날짜를 SUCCESS로 보정, 출석 +1, 스트릭 +1(단, 마일스톤 보상 코인은 없음). 복귀 대상일에는 일반 인증이 차단(409 RECOVERY_DAY_NO_CHECKIN)된다. |
...
[시작] 사용자: POST /api/personal/check-in  { lat, lng } + JWT
   │
   ▼
① 복귀 대상일 여부 확인 ──(오늘 마감 복귀 미션 존재)──▶ 409 RECOVERY_DAY_NO_CHECKIN ─▶[종료]
   │ (아님)
   ▼
...
※ ①: (F2 정책) 오늘이 복귀 대상일이면 일반 인증을 막고 복귀 수행이 곧 그날의 인증이 된다(이중 카운트 방지).
```
...
[사용자] POST /api/personal/recovery  { lat, lng }
   ...
   │ (마감 이내 & 반경 안)
   ▼
   미션 COMPLETED + 미인증일 SUCCESS 보정 + 오늘 SUCCESS 레코드 생성 + 코인 -50(클램핑)
   + 출석 +1 + 스트릭 +1(복귀일을 인증으로 간주, 마일스톤 보상은 없음) ─▶[종료]
```
