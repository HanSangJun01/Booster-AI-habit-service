...
> ## ⚠️ 이 계획은 일부 폐기됐다 (2026-08-27)
>
> 이 문서는 **복귀 미션 모델**(매일 판정, 실패 시 GPS 재인증, −50/−100 코인)을 전제로 작성됐다.
> 그 모델은 V14 마이그레이션에서 **폐지**되고 **주간 목표 + 구제권** 모델로 교체됐다.
> `RecoveryMission` 엔티티와 `recovery_missions` 테이블은 존재하지 않는다.
>
> **현재 기준선**: `docs/project-plan.md` §3 · `docs/api/MVP_API_SPEC.md` §5
>
> 아래 내용은 *2026-06-06 시점에 무엇을 어떤 근거로 정했는지*를 남기기 위해 보존한다.
> 구현 참조용으로 읽지 말 것.
