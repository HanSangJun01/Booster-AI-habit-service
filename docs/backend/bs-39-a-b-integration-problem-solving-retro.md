...
### B-1. 예치금 차감 TOCTOU — 사전검사와 차감의 트랜잭션 분리
**위치**: `CoinServiceAdapter.deduct()` → `CoinService.chargeStrict()`

- **문제**: B축 `deduct`가 A축 실구현에 연결되며, 무락 `getBalance` **사전검사**(트랜잭션 1) → `charge`(트랜잭션 2, 부족 시 예외 없이 **잔액까지만 클램핑**)의 2단계로 나뉘었다. 같은 유저가 서로 다른 두 챌린지에 **동시 참여**하면 둘 다 사전검사를 통과하고, 한쪽이 조용히 클램핑 차감된 채 CONFIRMED. 정산은 `totalPool = 예치금 × 참여자수`로 계산하므로 **장부상 풀 > 실제 징수** → 코인이 무에서 발행된다.
- **적용 방식**: A축 `CoinService.chargeStrict()` 신설 — `lockUser`(유저 행 PESSIMISTIC 락) **안에서** 잔액검사 → 차감 → `InsufficientCoinException`을 **단일 트랜잭션**으로 수행. 어댑터 `deduct`는 사전검사를 버리고 `chargeStrict`에 위임.
- **후보군**
  - 어댑터에 사전검사 유지(기존) — 검사·차감이 다른 트랜잭션이라 락이 풀려 TOCTOU
  - **차감측(A축 CoinService)에 strict 메서드 추가, 락 안에서 원자화** ✅
  - 참여 서비스에서 유저 행까지 추가 락 — 락 대상이 늘고 교착면 확대
  - 낙관적 버전(version 컬럼) — 재시도 로직 필요, 스키마 변경
- **선정 이유(장점)**
  - **TOCTOU 원천 차단**: 잔액검사와 차감이 같은 행 락 안에서 원자적 → 동시 두 요청이 직렬화되어 두 번째는 예외로 거절
...
## 관통 주제
- **A·B** = "신원과 금액은 서버가 원자적으로 확정한다" — JWT userId를 참여자로 해석해 위조 차단(A), 잔액검사+차감을 한 락에 묶어 무발행 차단(B).
