...
> **이 문서 = 1~7차 전체 종합 + 현재 상태 + 다음 단계.** (새 채팅/월요일 최종보고서용 핸드오프)
> 작성: 2026-06-25 / 개정: 2026-07-01(7차 추가) / 브랜치: `test/BS-30-backend-validation-a-axis`
> 회차별 상세는 `FINDINGS_1차~4차.md`, `FINDINGS_5차-출시검증.md`, `FINDINGS_6차-시나리오리뷰.md`, **`FINDINGS_7차-재검증.md`** 참고.
>
> ⚠️ **세 종류의 결과가 섞여 있으니 구분할 것:**
> - **1~5차 = 성능/부하 측정** → 결론: 성능·안정성 양호, 긴급 성능버그 없음(용량 산정이 과제).
> - **6차 = 시나리오/동시성 코드리뷰 + 테스트** → 결론: **정합성 버그 7~8건 발견(RED 증명).** 부하측정이 못 잡던 단일유저 동시요청·다일 상태머신 버그.
> - **7차 = 수정 후 재검증** → 결론: **6차 수정 정상(데드락 없음), 재검토서 신규 F1(HIGH) 등 발견·수정. 성능 회귀 없음, 커넥션풀 최적화 A/B 실증(+52% 처리량).**
...
| **7차** | **수정 후 재검증(코드리뷰 재발견 + 성능 재측정)** | 6차 수정 8종 정상·데드락 없음 확인. 신규 **F1(HIGH: 만료 시 새 스트릭 오삭제)** 등 8건 수정(32테스트 GREEN). 성능 재측정: 회귀 없음, **커넥션풀 10→30 A/B로 처리량 +52%·p95 −30% 실증**, 동시성 수정 실서버 5xx=0, soak 누수 없음 |
...
- **산출물(이 폴더):** `FINDINGS_1차~4차.md`, `FINDINGS_5차-출시검증.md`, **`FINDINGS_6차-시나리오리뷰.md`**, 본 종합요약.
  - 부하 스크립트: `a-axis-b-axis-load.js`(읽기, LOGIN_EMAIL 모드 있음), `a-axis-write-b-axis-load.js`(쓰기/로그인), `a-axis-stress-test.js`(고동시성 500VU), `a-axis-soak-test.js`(30m 지속)
...
docker run --rm -i -e BASE_URL=http://host.docker.internal:8080 -v ${PWD}:/scripts grafana/k6 run /scripts/a-axis-b-axis-load.js
