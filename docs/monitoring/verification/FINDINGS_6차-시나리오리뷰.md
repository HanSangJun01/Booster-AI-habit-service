...
## 5. 팀/제품 결정 (2026-07-01 확정)
- **B1** ✅ **결정됨**: (1) 잠정 스트릭(RECOVERY_PENDING) 동안 마일스톤 보상 **보류** — 복귀 확정 후에만 지급. (2) 7일 보상은 **반복 지급**(7·14·21…). → 코드: "확정 스트릭에만 지급 + 7일 주기 반복"으로 구현.
- **B4** ✅ **정리됨**: 코드 정상, 명세 문구 모순만 정정 완료(`docs/backend/plan-axis-a-backend.md` Phase 3 L207/L234 — "당일 추가 인증 불가" → "오늘 레코드 미생성, 단 오늘 일반 인증 별개 허용").
- **B2** ⏳ 결정 필요: 가입 당일 미인증 면제 정책 (명세 명문화). ※코드 off-by-one은 확정 버그라 수정 대상.
- **B3** ⏳ 결정 필요: 탈퇴(비활성) 후 차단 범위 — 로그인만? 전 API? 토큰 무효화? ※최소 active 가드는 수정 대상.
